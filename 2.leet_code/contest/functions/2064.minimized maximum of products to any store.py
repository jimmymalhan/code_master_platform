...
class Solution:
    def minimizedMaximum(self, n: int, quantities: List[int]) -> int:
        beg, end = 0, max(quantities) 
        while beg + 1 < end: # binary search
            mid = (beg + end)//2 # ceil
            if sum(ceil(i/mid) for i in quantities) <= n: # check if sum of ceil(quantities[i]/mid) <= n
                end = mid # update end
            else:
                beg = mid # update beg
...
def main():
    n = 6
    quantities = [11,6]
    print(Solution().minimizedMaximum(n, quantities)) # 3
