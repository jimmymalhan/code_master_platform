...
# recursive function to print hierarchy as tree
    def print_tree(self, id): 
        employee = self.api_fetcher(id)
        print(employee['name'] + ' - ' + employee['title'])
        for report in employee['reports']:
            print('   ' + self.print_tree(report)) #recursive function to print hierarchy as tree


def main():
    employee = Employee('', '', [])
    employee.print_hierarchy('A123456789') # call print_hierarchy function based on employee id
